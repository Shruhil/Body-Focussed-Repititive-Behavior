...
# Third LSTM block
x = Bidirectional(LSTM(32, return_sequences=False, recurrent_dropout=0.2))(x)
x = LayerNormalization()(x)
x = Dropout(0.3)(x)
...
callbacks = [
    ...
    ModelCheckpoint('best_lstm.h5', monitor='val_accuracy', save_best_only=True, mode='max', verbose=1)
]
...
print(f"\n📊 Improvement:")
print(f"   From baseline (18.15%) : {(accuracy - 0.1815)*100:+.2f}%")
print(f"{'='*70}")
...
plt.suptitle('LSTM Training History - All Metrics', fontsize=16, fontweight='bold', y=1.00)
plt.tight_layout()
...
colors = ['#3498db', '#2ecc71', '#9b59b6', '#e74c3c', '#f39c12', '#1abc9c', '#34495e']
...
ax.set_title('LSTM Model - Performance Metrics Comparison', fontsize=14, fontweight='bold', pad=20)
ax.set_ylim(0, 1.1)
ax.grid(axis='y', alpha=0.3, linestyle='--')
plt.xticks(rotation=0, ha='center')
plt.tight_layout()
plt.savefig('lstm_metrics_comparison.png', dpi=300, bbox_inches='tight')
print("✅ Saved: lstm_metrics_comparison.png")
plt.show()
...
metadata = {
    ...
    'improvement_from_baseline': float((accuracy - 0.1815) * 100),
